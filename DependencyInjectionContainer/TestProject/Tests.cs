...
namespace TestProject
{
    public class Tests
    {
        ...
        [Test]
        public void CycleDependencies()
        {
            DependencyConfig dependencies = new DependencyConfig();
            dependencies.Register<IDriver, Woman>(LifeTime.Instance);
            dependencies.Register<ICar, Bmw>(LifeTime.Instance);

            DependencyProvider provider = new DependencyProvider(dependencies);
            Woman service = (Woman)provider.Resolve<IDriver>();

            //IHuman->IAnimal (NOT NULL)
            Assert.IsNotNull(service.getValue());

            //IHuman->IAnimal->IHuman (NULL)
            Assert.IsNull(service.car.getValue());
